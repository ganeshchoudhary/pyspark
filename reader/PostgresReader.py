...
def postgress_df_read(spark: SparkSession, con=None) -> DataFrame:
    ...
    # url = "jdbc:postgresql://172.31.29.252:5432/partner_customer_loan_db"
...
if __name__ == '__main__':
    ...
    conf.set("spark.jars.packages", "mysql:mysql-connector-java:8.0.22,org.postgresql:postgresql:42.4.0,com.datastax.spark:spark-cassandra-connector_2.12:3.1.0")  # set the spark.jars
    conf.set("spark.sql.catalog.cass200","org.postgresql.Driver")
    spark = SparkSession \
        .builder.master('spark://3.6.75.0:7077').config(conf=conf).appName("Python Spark SQL basic example").getOrCreate()


    df = spark.read \
        .format("jdbc") \
        .option("url", "jdbc:postgresql://3.6.75.0:5432/partner_customer_loan_db") \
        .option("dbtable", "sonata_risk_mangement_transaction_table_v6") \
        .option("user", "ganeshc") \
        .option("password", "Gan35hC@8173") \
        .option("driver", "org.postgresql.Driver") \
        .load()
    df.show(n=10)
